...
void main() {
    ...
    for(int y=-1; y<=1; y++) {
        for(int x=-1; x<=1; x++) {
            ...
            min_dist = min(min_dist, dist*2.0); //get the closest distance //2.0 - scalar
        }
    }
    ...
    gl_FragColor = vec4(vec3(1.0-colour.r, 1.0 - colour.g, colour.b), 1.0);
}
